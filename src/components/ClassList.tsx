...
import type { HeightProp, MarginTopProp, SrcProp, WidthProp } from '@/types/ui/styled-props';
...
const ListThumbnailWrap = styled.div<SrcProp>`
  width: 30%;
  border-radius: 6px;
  background: ${({ src }) => (src ? 'url(' + src + ')' : 'white')} center no-repeat;
  background-size: cover;
  border: 1px dashed rgb(180, 180, 180);
  border-style: ${({ src }) => (src ? 'solid' : 'dashed')};
...
const ListInfoWrap = styled.div<WidthProp>`
...
const InfoSubTitle = styled.div<HeightProp>`
  line-height: 1.3;
  font-size: 14px;
...
const InfoHashTagWrap = styled.div<MarginTopProp>`
  display: flex;
  align-items: center;
  margin-top: ${({ mt }) => mt || '4px'};
`;
...
const InfoLocation = styled.div<{ mb?: string }>`
  display: flex;
  cursor: pointer;
`;
...
const ClassList = ({ v, on }: ClassListProps) => {
