...
const ClassListNothing = styled.div<{ height?: boolean }>`
  width: 100%;
  height: ${({ height }) => (height ? '70vh' : '74vh')};
  background: white;
  display: flex;
...
const DetailSearchList = ({ category }: { category: string }) => {
  const location = useAppStore((state) => state.user.result.location);
  const [classList, setClassList] = useState<ClassSummary[]>([]);
  const [mainLocation, setMainLocation] = useState('');
  const [pages, setPages] = useState(0);
  const [hasData, setHasData] = useState(true);
  const useSearchCategory = useLocation().state?.searchCategory as string | undefined;
