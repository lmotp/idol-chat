...
import Calendars from '@/components/Pickers/Calendars';
...
const ScheduleDay = styled.div<{ day?: number }>`
  ${({ day }) =>
    day === 0 &&
    css`
      color: red;
...
const MyClassSchedule = ({ myMeetinsList }: { myMeetinsList: ClassMeetingRecord[] }) => {
  const [meetingDayValue, setMeetingDayValue] = useState<Date | null>(new Date());
  const navigate = useNavigate();
  const [test, setTest] = useState<ClassMeetingRecord[]>([]);

  useEffect(() => {
    setTest(
      myMeetinsList.filter((v) => format(parseISO(String(v.day)), 'MM월 dd일') === format(meetingDayValue ?? new Date(), 'MM월 dd일')),
    );
  ...
  return (
    <MyClassScheduleContainer>
      ...
      <ScheduleWrap>
        <ScheduleDayWrap>
          <ScheduleDate>{format(meetingDayValue ?? new Date(), 'yyyy년 MM월 dd일 ')}</ScheduleDate>
          <ScheduleDay day={getDay(meetingDayValue ?? new Date())}>{dayArray[getDay(meetingDayValue ?? new Date())]}</ScheduleDay>
        </ScheduleDayWrap>
            ...
            {test.map((v) => {
              ...
              return (
                <List
                  key={classId._id}
                  onClick={() => {
                    navigate(`/pages/class/${classId._id}`);
                  }}
                >
                  <ListClassName>{classId.className}</ListClassName>
